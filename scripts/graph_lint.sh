...
# 中文條件詞用有界距離,且邊界排除多種句內標點,避免 .* 跨整段貪婪匹配誤殺 confirmed 邊。
...
while IFS=$'\t' read -r id arrow qual quote || [ -n "${id:-}" ]; do
  ...
  # malformed:缺箭頭/定性/引文(含純空白引文) → 不可靜默放行(confirmed 邊缺引文會逃過假清晰偵測)
  if [ -z "$arrow" ] || [ -z "$qual" ] || [ -z "$(printf '%s' "$q" | tr -d '[:space:]')" ]; then
    bad=$((bad+1))
    printf '%-26s %-7s %-12s %s\n' "$id" "${arrow:-?}" "${qual:-?}" "🔴 malformed:缺箭頭/定性/引文(含純空白),須補齊四欄"
    continue
  fi
